...
function ClientsTab({ clients, onConfirmBooking }: { clients: Client[]; onConfirmBooking: (clientId: number) => void }) {
        ...
        {(clientsSubTab === 'pending' ? pendingClients : clients).map((client, index) => (
          <motion.div
            ...
            className={`liquid-glass-admin p-3 flex items-center justify-between rounded-xl ${
              ...
              client.status === 'cancelled' ? 'border-l-4 border-l-[#ef4444] opacity-60' : ''
            }`}
          >
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="flex-1">
                ...
                {client.status === 'cancelled' && client.cancelReason && (
                  <p className="text-[#ef4444] text-[10px] mt-0.5">
                    ❌ {client.cancelReason}
                  </p>
                )}
                {(client.username || client.userId) && (
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const historyResponse = await apiClient.getBookingHistory();
        if (historyResponse.success) {
          const bookings = historyResponse.bookings.map((b: any) => ({
            id: b.id,
            name: b.client_name,
            time: b.slot_time,
            date: b.day_date,
            status: b.is_cancelled ? 'cancelled' : 'confirmed',
    ...
    <div className="liquid-glass-admin p-4 w-full">
      ...
      {loading && (
        <div className="text-center py-8">
          <p className="text-[#9e8476] text-sm">Загрузка данных...</p>
        </div>
      )}
